<?php
    ...
    function get_Data_From_Pd_Ord_By_Month($user_sbu)
    {
        try
        {
            ...
            $strSql .= "WHERE YEAR(`basic start date`) = YEAR(CURDATE()) ";
            ...
            $strSql .= "GROUP BY cYear, cMonth ";
            $strSql .= "ORDER BY cYear, cMonth ";            
            //echo $strSql . "<br>";
    ...
    function get_Data_From_Pd_Ord_By_Grp($user_sbu)
    {
        try
        {
            ...
            $strSql .= "FROM tpdt_trn_pd_sch T ";
            $strSql .= "JOIN tpdt_mas_pd_data M ON M.pd_code = T.`Material Number` ";
            $strSql .= "WHERE YEAR(T.`Basic start date`) = YEAR(NOW()) ";
    ...
    function get_Data_From_Pd_Ord_Top_20($user_sbu)
    {
        try
        {
            ...
            $strSql = "SELECT `Material description`, SUM(`Actual quantity`) as 'Qty' "; 
            $strSql .= "FROM tpdt_trn_pd_sch T ";
            $strSql .= "JOIN tpdt_mas_pd_data M ON M.pd_code = T.`Material Number` ";
            $strSql .= "WHERE YEAR(`basic start date`) = YEAR(CURDATE()) ";
